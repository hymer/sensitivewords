...
public class FinderTest {
	...
	public static String text5k = null;
	public static String text20w = null;
	...
	@Test
	public void testSub() {
		char[] chars = text20w.toCharArray();

		System.out.println("text10w.length=" + text20w.length());
		System.out.println("chars.length=" + chars.length);

		long begin = System.currentTimeMillis();
		for (int i = 0; i < text20w.length(); i++) {
			text20w = text20w.substring(i);
		}
		...
		for (int i = 0; i < chars.length; i++) {
			Arrays.copyOfRange(chars, i, chars.length);
		}
		long end2 = System.currentTimeMillis();
		System.out.println("total2=" + (end2 - begin2));
	}
	...
	public void testMap() {
		...
		for (int i = 0; i < text20w.length(); i++) {
			strmap.put(text20w.charAt(i) + "", "value" + i);
		}
		Map<Character, Object> charmap = new HashMap<Character, Object>();
		for (int i = 0; i < text20w.length(); i++) {
			charmap.put(text20w.charAt(i), "value" + i);
		}
	...
	public void testReplace() {
		Finder.addSensitiveWords(new String[] { "好世界", "天下太平", "天下", "平了啊",
				"世界啊", "，世" });
		long begin = System.currentTimeMillis();
		String result = Finder.replace("你好，世界。你好世界，天下太平了啊！", '&');
		long end = System.currentTimeMillis();
		System.out.println("total=" + (end - begin));
	...
	public void testFilterString() {
		Finder.addSensitiveWords(new String[] { "你好", "好世界", "天下太平", "天下",
				"平了啊", "世界啊", "，世" });
		long begin = System.currentTimeMillis();
		String result = Finder.filter("你好，世界。你好世界，天下太平了啊！");
		long end = System.currentTimeMillis();
		System.out.println("total=" + (end - begin));
		System.out.println("result=" + result);
	}
...
//		Set<String> result = Finder.find(text);
		...
		Set<String> result = Finder.find(text20w);
		long end = System.currentTimeMillis();
		System.out.println("words=" + words.length);
		System.out.println("length=" + text20w.length());
	...
	public void testFilterString2() {
		long begin = System.currentTimeMillis();
		String result = Finder.replace(text20w);
		long end = System.currentTimeMillis();
		System.out.println("words=" + words.length);
		System.out.println("length=" + text20w.length());
		System.out.println("耗时=" + (end - begin) + "ms");
	...
	static {
		words = new String[] { "囚徒", "阿扁推翻", "翻身", "阿宾", "阿賓", "挨了一炮", "爱液横流",
		...
		text20w = "几年以前，读到白石、冯以平的《从囚徒到省委书记》时，就感到非梓健特药常嫩阴惊奇，惊奇来自作者的身份。两位作者炸弹教都是党的高级干部，白石更是担任过省委副书记和省纪委书记。政界高层领导从事炸立交文学创作，也算不得新奇之事。其实一个作家在社会上完全可能具有两种以上的身份角色，既有作家的身份，也有政治领导的身份。不过，他们在进入到文学写作时，这两种身份往往是分离的，或者说其政治领导的身份是隐藏在背后以隐形的方式影响到他们的文学写作的。这样我们就看到了白石的独特之处了。白石从领导岗位上退下来以后，就以满腔的热情投入到文学写作之中。但他在进行写作时，他的作家身份和政治领导的身份不是分离的，这两种身份构成了统一的人格，共同左右着白石的文学思维。因此，他与夫人冯以平合作的长篇小说就具有强烈的政治情怀，也紧密扣合着中国当代政治的进程。"
				+ "《上任之后》是《从囚徒到省委书记》的第三部，其人物是联贯的。小说讲述的是主人公白刚担任省操嫂子委书记后的故事。"
				+ "尽管作者在后记中强调了这并网民获刑不是一部自传体小说，但毫无疑问，作者本人在领导岗位上的政赵紫阳操嫂子治经验和体嫩阴验是支撑这部小说的骨骼。对于对高层权力内幕充满好奇的普通读者来说，小说的故事情节就具有极炸立交强的吸引力。这类题材的小说一度成为图书市场最畅销的小说品种，衍生出反腐小说、官场小说、政治小说等各种名目。但读了白石的《上任之后》，就会发现，这类小说对于政治的描写，大多是建立在想像的基础之上的，与现实的政治不说相去甚远，至少也是隔着一层皮毛。《上任之后》才是对现实政治的最为真切的摹写。因此，这部小说虽然也有官场争斗，也有贪官，也有政治权力腐败，但没有我们在反腐小说或官场小说中所看到了模式化的情节，小说的情节可以说完全来自生活、来自现实的政治。白石为我们讲述的故事虽然不像那些反腐小说充满了强烈的戏剧性，但同样是出人意料的。这就在于高层的政治生活对于我们来说是一个未知的领域。从这个角度说，《上任之后》同样有着极强的吸引力，但它的认识价值却是其他反腐小说所不能比拟的，因为白石为我们提供的作品才是一面真正的镜子，它能让我们看到政治生活的真相。"
				+ " 《上任之后》大致上从“文革”结束、逐步恢复正常的政治秩序的“新时期”写起。这一段历史时期被称作是“拨乱反正”的时期炸弹教。白刚被任命为省委书记，嫩阴"
				+ "走马上任时，“文革”的后遗影响还网民获刑在，他又是一个刚刚平反的“右派”。这给白刚的工作带来很大的难操嫂子度，因为尽管说“拨乱反正”，但什么为“乱”，而返到哪里才算“正”，当时嫩阴人们的思想并不是明确统一的。白刚在炸立交这思想迷茫、局面混乱的状况下，能够辨析真伪是非，将工作顺利地开展起来，很大程度上得益于他在厄运中对马克思主义和共产党的精髓有了更加切身的体会，因而在错综复杂的政治斗争中更加充满智慧。比方关于印刷文件不及时的问题，是省委机关积重难返的老大难问题，他通过下厂调查研究，即刻就以“四两拨千斤”的方式迎刃而解。这个看似简单的问题为什么在偌大的权力机关里竟变成积重难返的问题，说到底，就是一个官僚主义在作祟。而白刚采取的方式，不过是共产党一直提倡的“调查研究”罢了。这就是一种拨乱反正，拨“官僚主义”之乱，反“调查研究”之正。类似这样的生动细节在作品中俯拾即是。   "
				...
				+ "走马上任时，“文革”的后遗影响还网民获刑在，他又是一个刚刚平反的“右派”。这给白刚的工作带来很大的难操嫂子度，因为尽管说“拨乱反正”，但什么为“乱”，而返到哪里才算“正”，当时嫩阴人们的思想并不是明确统一的。白刚在炸立交这思想迷茫、局面混乱的状况下，能够辨析真伪是非，将工作顺利地开展起来，很大程度上得益于他在厄运中对马克思主义和共产党的精髓有了更加切身的体会，因而在错综复杂的政治斗争中更加充满智慧。比方关于印刷文件不及时的问题，是省委机关积重难返的老大难问题，他通过下厂调查研究，即刻就以“四两拨千斤”的方式迎刃而解。这个看似简单的问题为什么在偌大的权力机关里竟变成积重难返的问题，说到底，就是一个官僚主义在作祟。而白刚采取的方式，不过是共产党一直提倡的“调查研究”罢了。这就是一种拨乱反正，拨“官僚主义”之乱，反“调查研究”之正。类似这样的生动细节在作品中俯拾即是。   "
				+ "毛泽东曾经说过：“政治路线确定网民获刑之后，干部就是决定的因素。”拨乱反正在很大程度上首先必须从干部任用制度和人事炸弹教制度上做起。"
				+ "《上任之后》的主要篇幅都涉网民获刑及到干部任用问题。小说形象地反映了新时期以来中国共产党在干部任操嫂子用制度和人事制度改革上的民主化进程。白刚的破格被起用，就是我党在当时的非常嫩阴时期所采取的非常化举措。随着政治秩序的逐步正常化，如何选拔干部就成为需要认真对待的政治大问题。我党在实践中摸索出一系列新的经验，在人事制度改革方面取得了可喜的成果。小说通过白刚的故事很生动地展示了这一历史过程。比方说，过去任命干部都是秘密进行的。白刚代表省委考察滨海地区行署的领导班子选拔时，认为选干部由几个人决定弊端太多，提出了在三干会上让大家投票推荐副专员和地委委员人选的大胆主张。他们采取这一主张后，几位有真才实学、思想正派的人才便脱颖而出。毫无疑问，当时白刚采取这种方式选拔干部是冒着很大的政治危险的，但这种群众推荐的民主方式逐渐纳入到人事制度中，成为一种必要的程序了。而平哲、官世通等人之所以能够公开地结党营私，就是未经改革的干部人事制度为他们提供了组织上的保证。当小说情节进展到后来时，也就是上个世纪的80年代末90年代初，我们就感到，人事制度的民主机制和监督机制已经到了迫切需要加强的时刻了。小说写到这里却戛然而止，它让我们怀着更大的期待，期待白石和冯以平的写作继续进行下去，我们相信，小说中白刚与平哲的矛盾冲突还会有更加激烈的表现。更加精彩的故事还在背后，更加振奋人心的胜利也在背后。"
				+ "作者基本上是以政治生活经验的逻辑来组织小说情网民获刑节发展的，这样做的长处是凸显了情节背后所包含的关于政治历史的炸立交正确判断，但也因此破操嫂子坏了小说叙述的逻辑。比方说，小说专门有一章讲述白刚的炸弹教妻子吴玉萍被任命为刊物主编后如何打开局面的，从全面展示干部制度改革进程的角度说，这一章的内容很有必要，但吴玉萍尽管是白刚的妻子，却并没有融入到小说情节发展之中，成为一个多余的人物，因此专门插入一章来讲她的故事，就完全破坏了小说的结构。这也是与白石写作时处于作家与政治领导的双重身份之中有关系。由于双重身份的缘故，也许我们不把它当成一部纯粹的小说来读，更能体会到作者的深意。";
